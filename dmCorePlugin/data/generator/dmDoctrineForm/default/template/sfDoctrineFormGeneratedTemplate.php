...
<?php foreach ($this->getOneToOneRelations() as $relation): ?><?php if($relation['alias'] === 'Translation') continue;?>
		if($this->needsWidget('<?php echo $this->underscore($relation['local']) ?>')){
			...
			$this->setValidator('<?php echo $this->underscore($relation['local']) ?>', new sfValidatorDoctrineChoice(array('multiple' => false, 'model' => '<?php echo $relation['table']->getOption('name')?>', 'required' => <?php $options =  $this->getValidatorOptionsForColumn($column);echo (isset($options['required']) && $options['required']==='true')?'true':'false'?>)));
		}
<?php endforeach; ?>
